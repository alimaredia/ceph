...
int main() {
  ...
  std::cout << ref->instance_labels << " l_rgw_put_b after inc 500 is " << ret << std::endl;
  cache.set_counter(key1, l_rgw_put_b, 300);
  ret = cache.get_counter(key1, l_rgw_put_b);
  std::cout << ref->instance_labels << " l_rgw_put_b after set 300 is " << ret << std::endl;
  cache.dec(key1, l_rgw_put_b, 50);
  ret = cache.get_counter(key1, l_rgw_put_b);
  std::cout << ref->instance_labels << " l_rgw_put_b after dec 50 is " << ret << std::endl;
  std::cout << std::endl;
  ...
  std::cout << ref2->instance_labels << " initial l_rgw_get_b is " << ret << std::endl;
  cache.set_counter(key2, l_rgw_get_b, 400);
  ret = cache.get_counter(key2, l_rgw_get_b);
  std::cout << ref2->instance_labels << " l_rgw_get_b after set 400 is " << ret << std::endl;
  cache.inc(key2, l_rgw_get_b, 1000);
  ret = cache.get_counter(key2, l_rgw_get_b);
  std::cout << ref2->instance_labels << " l_rgw_get_b after inc 1000 is " << ret << std::endl;
  cache.dec(key2, l_rgw_get_b, 500);
  ret = cache.get_counter(key2, l_rgw_get_b);
  std::cout << ref2->instance_labels << " l_rgw_get_b after dec 500 is " << ret << std::endl;

...
JSONFormattable DriverManager::get_default_sal_config() {
  ...
  if (config_store == "rados") {
    ...
    // QUESTION: should we be getting the config_uri or db_dir here?
    // OBSERVATION: should this be the config stores config uri?
    // we should have the config_uri point to a dir and then create the file the same way we have db_dir
    //const auto& default_config_uri = g_conf().get_val<std::string>("dbstore_config_uri");
    // for now I'm setting this in the config for radosgw-admin calls to get_raw_storage

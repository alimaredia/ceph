...
class RGWSyncShardMarkerTrack {
  struct marker_entry {
    ...
    marker_entry() : pos(0) {}
    marker_entry(uint64_t _p, const real_time& _ts, real_time _lu) : pos(_p), timestamp(_ts), last_update(_lu) {}
  };
...
public:
  ...
  void try_update_high_marker(const T& pos, int index_pos, const real_time& timestamp) {
    finish_markers[pos] = marker_entry(index_pos, timestamp, timestamp);
  }

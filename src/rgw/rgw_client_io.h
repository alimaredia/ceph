...
public:
  ...
  virtual size_t send_body(const DoutPrefixProvider *dpp, const char* buf, size_t len) = 0;
...
public:
  ...
  size_t send_body(const DoutPrefixProvider *dpp, const char* const buf,
    ...
    return get_decoratee().send_body(dpp, buf, len);
  }
...
public:
  ...
  virtual size_t write_data(const DoutPrefixProvider *dpp, const char *buf, size_t len) = 0;
};
...
class StaticOutputBufferer : public std::streambuf {
  ...
  int sync() override {
    ...
    sink.write_data(dpp, std::streambuf::pbase(), len);
...
public:
  explicit StaticOutputBufferer(const DoutPrefixProvider *dpp, BuffererSink& sink)
    : dpp(dpp), sink(sink) {
